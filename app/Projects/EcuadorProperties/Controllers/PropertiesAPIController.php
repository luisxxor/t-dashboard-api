 ...
 * @package App\Projects\EcuadorProperties\Controllers
 */
class PropertiesAPIController extends AppBaseController
{
     ...
     *
     * @OA\Get(
     *     path="/api/ecuador_properties/filters/publication_type",
     *     operationId="getPublicationTypeFilterData",
     *     tags={"Ecuador Properties"},
     *     summary="Return the necessary data for property type filter",
     *     @OA\Response(
     *         response=200,
     ...
     *
     * @OA\Post(
     *     path="/api/ecuador_properties/search",
    ...
    public function order( Request $request )
    {
        ...
        if ( empty( $order ) === true ) {
            ...
            $order = $this->orderRepository->create( [
                'user_id'               => $user->id,
                'search_id'             => $searchId,
                'project'               => config( 'multi-api.ec-properties.backend-info.code' ),
        ...
        if ( $user->hasPermissionTo( 'release.order.without.paying' ) === true ) {
            ...
            $guzzleClient->sendAsync( new GuzzleRequest(
                'GET',
                route( 'api.' . config( 'multi-api.ec-properties.backend-info.generate_file_url' ), [], false ),
    ...
    public function generatePropertiesFile( Request $request )
    {
        ...
        try {
            ...
            $filesInfo[] = $this->googleStorageHandler->uploadFile( config( 'app.ec_export_file_bucket' ), $path, $order->total_rows_quantity );
